...
func sendMsgAndSave(userId int64, msg []byte) {
	...
	//timeStr := "now time is :" + time.Now().String()
	//timeBytes := []byte(timeStr)
	//如果在线，需要即时推送
	if ok {
		//node.DataQueue <- timeBytes
		node.DataQueue <- msg
	}
	...
	zap.S().Info("存储聊天记录的key为:", key)

	//创建记录
	res, err := initialize.RedisDB.ZRevRange(ctx, key, 0, -1).Result()
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	ress, e := initialize.RedisDB.ZAdd(ctx, key, &redis.Z{score, msg}).Result()
	...
	_, expireErr := initialize.RedisDB.Expire(ctx, key, expirationTime).Result()
	if expireErr != nil {
...
func RedisMsg(userIdA int64, userIdB int64, start int64, end int64, isRev bool) []string {
	...
	zap.S().Info("userIdA:", userIdA, "userIdB:", userIdB)
	//拼接key
	var key string
	if userIdA > userIdB {
		key = "msg_" + targetIdStr + "_" + userIdStr
	...
	var err error
	if isRev {
		rels, err = initialize.RedisDB.ZRange(ctx, key, start, end).Result()
	} else {
		rels, err = initialize.RedisDB.ZRevRange(ctx, key, start, end).Result()
	}
	if err != nil {
...
// RecordPersistence 聊天记录持久化,存储到数据库里
	...
	for {
		...
		case <-ticker.C:
			...
			go func(keys []string) {
				...
				// 控制并发，10并发缓冲区满，G陷入阻塞
				// 直接定义一个空结构体没有意义，但是struct{}{}可以作为channel通信的信号量
				concurrency <- struct{}{}
				defer func() {
				...
				for _, key := range keys {
					// 从小到大排序
					chatRecords, err := initialize.RedisDB.ZRange(ctx, key, 0, -1).Result()
					if err != nil {
					...
					//开启事务，调用global.DB.Begin()之后就不能再用global.DB去执行crud，因为底层的db处于busy状态
					tx := global.DB.Begin()
					...
					for _, bg := range backMsg {
						if err := initialize.RedisDB.ZRem(ctx, key, bg).Err(); err != nil {
